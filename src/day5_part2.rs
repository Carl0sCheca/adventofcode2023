        ...
        let final_result = (0..seeds.len())
            ...
            .map(|i| {
                ...
                (start..end)
                    ...
                    // .into_iter()
                    .map(|seed| {
                        results.iter().fold(seed, |check_value, result| {
                            result
                                .iter()
                                .filter(|r| check_value >= r.to && check_value < r.to + r.size)
                                .next()
                                .and_then(|index| Some((check_value - index.to) + index.from))
                                .unwrap_or(check_value)
                        })
                    })
                    .min()
                    .unwrap()
